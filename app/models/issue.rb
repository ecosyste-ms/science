class Issue < ApplicationRecord
  ...
  GOOD_FIRST_ISSUE_LABELS = [":beginner: good first issue",
  ...
  scope :has_good_first_issue_label, -> { label(GOOD_FIRST_ISSUE_LABELS) }
  scope :good_first_issue, -> { has_good_first_issue_label.where(pull_request: false, state: 'open').where('issues.updated_at > ?', 2.years.ago)  }
