...
A rate-limited batch retains its pending projects and schedules one shared retry. New coverage checks join that pending set during the cooldown. Local cloning and SWHID calculation continue while API calls are paused. Existing queued `FetchSwhidWorker` jobs use the batch path when they run after deployment; no database migration or queue cleanup is needed.
