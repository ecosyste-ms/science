...
The `journal` process runs `bundle exec rake swhids:consume`. It requires `SWH_JOURNAL_BROKERS` (comma-separated broker addresses), `SWH_JOURNAL_GROUP_ID`, `SWH_JOURNAL_USERNAME` and `SWH_JOURNAL_PASSWORD`. Use the broker addresses and consumer-group prefix supplied by SWH. Connections use TLS with SCRAM-SHA-512 authentication; the API token is separate. See the [journal authentication documentation](https://docs.softwareheritage.org/devel/swh-journal/journal-clients.html#authentication).

The consumer subscribes only to `swh.journal.objects.origin_visit_status`. It decodes MessagePack timestamps and binary snapshot identifiers, then filters terminal Git visits against pending requests' submitted origin URLs. Each batch contains at most 1,000 messages and flushes after one second even when traffic is low. Repeated origins within a batch produce one lookup. Matching projects are read in batches of 1,000 and checks are queued in bulk on `swh_api`.

A matching event schedules verification after one minute to allow SWH's request status to catch up. The worker checks the saved request ID and exact SWHIDs through the existing archiver. Events do not establish contributions on their own. Duplicate or older events cannot repeatedly advance the same request's next check, and queued events for replaced or completed requests are ignored. The six-hour polling fallback remains in place, including when an event arrives before our submission response is stored or before SWH reports task completion.

Kafka offsets are committed after all matching jobs in the batch have been queued. Queue or database errors stop the process without acknowledging the batch. Restart it under a process supervisor; replay can enqueue duplicates, which the worker handles using the event timestamp stored with the request. Invalid terminal-event payloads also stop consumption and require investigation. SIGINT and SIGTERM flush the current batch and close the consumer.

Keep the consumer-group ID stable across restarts. A new group starts at the latest offsets; an existing group resumes its committed offsets. Offsets outside Kafka's retained history also reset to the latest position, with polling covering outstanding requests. Multiple consumers in the same group share partitions.

This draft leaves the database schema unchanged. Pending-origin matching uses `lower(swhids->'archival'->>'origin')` with a pending-status filter, which currently has no supporting index. Batching limits query frequency and memory, but high-throughput deployment needs an indexed lookup or another measured matching strategy. Kafka authentication and throughput against SWH's production topic still need verification before enabling the process.
