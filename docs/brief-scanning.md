...
Eligible projects are visible, have repository metadata, have a science score above zero, and have no stored Brief dependency result. `COHORT` accepts `all`, `joss`, or `non_joss`. `LIMIT` defaults to 50 for the rake task.
...
Clone, timeout, and parse failures store an error and attempt time in `projects.brief`, which keeps repeated cohort runs from retrying the same failed repository. Avoid enqueueing the same cohort again while its earlier jobs remain queued. A project has no `brief` value until its job starts, so concurrent task runs can enqueue duplicate jobs. Direct dependencies from Brief are used by the dependency indexer when stored repos manifest data has no usable direct dependencies.
